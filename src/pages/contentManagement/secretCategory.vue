<template>
  <div class="container secretCategory">
    <el-form :inline="true" :model="filterForm">
      ...
      <el-form-item>
        <el-button @click="categoryList">查询</el-button>
      </el-form-item>-->
...
<script>
  export default {
    ...
    methods: {
      ...
      //获取课程分类列表
      categoryList() {
        this.$http('/admin/cheats/category', {
          page: this.pageNum,
          size: this.pageSize,
      ...
      save() {
        ...
        this.$http('/admin/cheats/modifyCategory', params).then(res => {
          if (res.code == 0) {
      ...
      edit(item) {
        if(item){
          ...
          this.editId = item.id;
          this.editForm.categoryName = item.name;
      ...
      remove(pkid){
        this.$confirm('是否删除该分类','提示',{
          ...
          this.$http('/admin/cheats/deleteCategory',{categoryId :pkid}).then(res=>{
            if(res.code == 0){
