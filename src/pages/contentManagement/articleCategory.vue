<template>
  <div class="container articleCategory">
...
<script>
  export default {
    ...
    methods: {
      ...
      //获取文章分类列表
      categoryList() {
        this.$http('/admin/article/category', {
          page: this.pageNum,
          size: this.pageSize,
      ...
      save() {
        ...
        this.$http('/admin/article/modifyCategory', params).then(res => {
          if (res.code == 0) {
      ...
      remove(pkid){
        this.$confirm('是否删除该分类','提示',{
          ...
          this.$http('/admin/article/deleteCategory',{categoryId:pkid}).then(res=>{
            if(res.code == 0){
