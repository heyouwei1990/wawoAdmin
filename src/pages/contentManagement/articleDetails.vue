<template>
  <div class="container articleDetails">
    <el-form ref="form" :model="form" :rules="rules" label-width="90px">
      <el-row>
        <el-col :span="6">
          <el-form-item label="文章分类" prop="c_category_id">
            <el-select v-model="form.c_category_id" placeholder="文章种类">
              <el-option v-for="(item,index) in articleCategory" :key="index" :label="item.name" :value="item.id">
              </el-option>
            </el-select>
          </el-form-item>
        </el-col>
        ...
        <el-col :span="6">
          <el-form-item label="原价" prop="orig_price" >
            <el-input v-model="form.orig_price" placeholder="请输入文章原价"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6" >
          <el-form-item label="现价" prop="price" >
            <el-input v-model="form.price" placeholder="请输入文章现价"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="会员价" prop="vip_price">
            <el-input v-model="form.vip_price" placeholder="请输入文章会员价" ></el-input>
          </el-form-item>
        </el-col>
        <!--<el-col :span="6">
        ...
        <el-col :span="6">
          <el-form-item label="推荐" prop="is_popular">
            <el-select v-model="form.is_popular " placeholder="推荐">
              ...
              <el-option label="文章推荐" :value="1"></el-option>
              <el-option label="首页推荐" :value="2"></el-option>
              <el-option label="首页、文章推荐" :value="3"></el-option>
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="12">
        ...
        <el-col :span="24">
          <el-form-item label="文章封面:" prop="thumbnail">
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        rules:{
          ...
          c_category_id:[{required:true,message:'请选择文章分类',trigger:'change'}],
          orig_price:[{required:true,message:'请输入文章原价',trigger:'blur'},{validator:validatePrice,trigger:'blur'}],
          price:[{required:true,message:'请输入文章现价',trigger:'blur'},{validator:validatePrice,trigger:'blur'},{validator:validateNotFree,trigger:'blur'}],
          vip_price:[{required:true,message:'请输入文章会员价',trigger:'blur'},{validator:validatePrice,trigger:'blur'}],
          status:[{required:true,message:'请选择文章状态',trigger:'change'}],
          thumbnail:[{required:true,message:'请上传文章封面',trigger:'blur'}],
          summary:[{required:true,message:'请选择文章摘要',trigger:'blur'}],
        ...
        myConfig:{
          ...
          serverUrl: window.URLCONFIG.baseUrl+'/api/img/insertImage?folder=articleDetail',
    ...
    methods: {
      init(){
        ...
        this.$http('/admin/article/detail',{
          contentId:this.pkid
        }).then(r=>{
          if(r.code==0){
      ...
      submit(){
        this.$refs['form'].validate(valid=>{
          if(valid){
            ...
            this.$http('/admin/article/createArticle',{
              ...params,
              content_id:this.pkid
            }).then(r=>{
              if(r.code==0){
                this.$message.success('保存成功！');
                this.$router.replace({path:'/articleManagement'});
              }
            })
          }
        })
